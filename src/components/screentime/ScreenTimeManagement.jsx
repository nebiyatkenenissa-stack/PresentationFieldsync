...
function ScreenTimeManagement({
  ...
  // Employee options for manager filter
    ...
    if (newLimit !== null) {
      ...
      if (limit >= 4 && limit <= 12) {
        ...
        try {
          ...
          if (online) {
            try {
              await sendScreenTimeUpdateToServer(screenTimeId, limit, user.employeeId);
              // Success
              if (addNotification) {
                addNotification(
                  user.id,
              ...
              await db.screen_time.update(screenTimeId, { synced: false });
              syncQueue.add({
                type: 'screen_time_update',
                id: screenTimeId,
                data: { id: screenTimeId, limit, verified: true, verifiedBy: user.employeeId }
              });
              setPendingCount(syncQueue.count());
              alert('⚠️ Limit saved locally but server sync failed. Will retry later.');
              if (addNotification) {
                addNotification(
                  user.id,
                  '⚠️ Sync Failed',
                  `Screen time limit updated locally but could not reach server.`,
                  'warning'
                );
              }
            }
