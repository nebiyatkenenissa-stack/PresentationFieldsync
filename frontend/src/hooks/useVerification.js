...
// Minimum gap between verifications (1 minute). The next verification can
// never appear sooner than this after the previous one.
const MIN_VERIFICATION_INTERVAL_MS = 60 * 1000;
...
export function useVerification(officerId, officerName) {
  ...
  const scheduleNext = useCallback(() => {
    ...
    // Never schedule sooner than the 1-minute minimum interval.
    const delay = Math.max(getRandomDelay(), MIN_VERIFICATION_INTERVAL_MS);
    const target = Date.now() + delay;
    setNextVerificationAt(target);
  ...
  const handleAnswer = useCallback(async (answerData) => {
    ...
    // Explicit failures (timeout / skipped) always register as a warning for the supervisor
    const isExplicitFailure = answerData.success === false;
    const { score: trustScore, penalties } = isExplicitFailure
      ? { score: 0, penalties: ['No response within the allowed time'] }
      : calculateTrustScore(
    ...
    const historyEntry = {
      ...
      score: isExplicitFailure ? (answerData.score ?? 0) : trustScore,
      penalties: isExplicitFailure ? (answerData.penalties || penalties) : penalties,
      questionId: currentQuestion.id,
      success: isExplicitFailure ? false : trustScore >= 60,
      message: isExplicitFailure
        ? (answerData.message || '⚠️ Verification warning')
        : (trustScore >= 60 ? '✅ Verification passed!' : '⚠️ Verification warning'),
      synced: false
    };
  ...
  useEffect(() => {
    ...
    setNextVerificationAt(Date.now() + 30 * 1000);
    timerRef.current = setTimeout(() => {
